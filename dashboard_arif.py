...
st.header('Dicoding Collection Dashboard :sparkles:\n by Muhammad Arif Fajriyansah')

tabs = st.tabs(["Daily Orders", "Best & Worst Performing Product", "Customer Demographics","Best Customer Based on RFM Parameters" ])
...
with tabs[0]:
    ...
    with col2:
        # Assuming format_currency is a function that works correctly
        total_revenue = format_currency(daily_orders_df['revenue'].sum(), "AUD", locale='es_CO')
        st.metric("Total Revenue", value=total_revenue)

    # Convert the "order_date" and "order_count" columns to lists or NumPy arrays
    order_dates = daily_orders_df["order_date"].tolist()
    order_counts = daily_orders_df["order_count"].tolist()
...
with st.container():
    st.caption('Copyright (c) Muhammad Arif Fajriyansah 2023')